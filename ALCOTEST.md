...
| `src/alcotest-engine/pp_intf.ml`   | Extend the tag polymorphic variant                                                    |
| `src/alcotest-engine/pp.ml`        | Five branches: colour, label, error-pretty, tag-of-result, compact char               |
...
| `test/e2e/alcotest/passing/dune.inc`                    | Auto-regenerated by `gen_dune_rules.exe` to wire the new test into the `runtest` alias |
...
is augmented. The e2e regression test lives inside alcotest's own test
harness and guards against accidental removal of the when-guard.
...
The patch is small — 11 added lines across 4 files under
`src/alcotest-engine/` for the feature itself, plus a tiny
`test/e2e/alcotest/passing/incomplete_in_test.ml` + `.expected` regression
guard (and the mechanical `dune.inc` regeneration that pulls it into
alcotest's own e2e harness). No refactors, no public-API changes, so
